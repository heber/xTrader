...
{
public:
	CtpTraderSpi(CThostFtdcTraderApi* api):pUserApi(api){m_bReconnect=FALSE; m_iRequestID =0; m_ifrontId=-1; m_isessionId=-1;}
   ~CtpTraderSpi();
...
public:
	...
	///请求查询投资者品种/跨品种保证金
	void ReqQryInstGroupMargin(TThostFtdcInstrumentIDType instId);
	
	///请求查询交易所保证金率
	void ReqQryExhMgr(TThostFtdcInstrumentIDType instId);
	
	///请求查询交易所调整保证金率
	void ReqQryExhMgrAdjust(TThostFtdcInstrumentIDType instId);
	...
	//撤单
	void ReqOrderCancel(TThostFtdcOrderSysIDType OrdSysID);
	void ReqOrderCancel(TThostFtdcInstrumentIDType instId,TThostFtdcOrderRefType OrdRef);
...
public:
	...
	int	 m_ifrontId;
	int	 m_isessionId;
	TThostFtdcOrderRefType  m_sOrdRef;
